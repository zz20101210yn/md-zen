...
public class TestSM4_CSV_ID_NO {
	public static void main(String[] args) {
        ...
        String filePath = "D:\\idno\\2monfull.csv";
        //-- 点石成金 犇牛战法 量价趋势擒龙 成林俱乐部 私颜社 主力掘金 万维龙头战法
        
        File file = FileUtil.file(filePath);
        
        try {
        	final BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream("D:\\idno\\chg\\2monfull.csv"), StandardCharsets.UTF_8));
			FileUtil.readLines(file, Charset.forName("utf-8"), new LineHandler() {
			    ...
			    public void handle(String line) {
			        ...
			        if(lineNum != 1) {
			        	try {
				        	if(parts.length == 4) {
				        		writer.write(lineString + "," + sm4.decryptStr(parts[3]));
				        	}else {
