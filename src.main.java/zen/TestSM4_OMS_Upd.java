...
public class TestSM4_OMS_Upd {
	...
	public static void main(String[] args) {
        ...
        String filePath = "D:\\idno\\OMS12.csv";
        ...
        try {
        	...
        	final BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream("D:\\idno\\chg\\OMS12.csv"), StandardCharsets.UTF_8));
			FileUtil.readLines(file, Charset.forName("utf-8"), new LineHandler() {
			    ...
			    public void handle(String line) {
			        ...
			        if(lineNum != 1) {
			        	try {
				        	if(parts.length >= 4 && StringUtils.isNotBlank(parts[2]) && StringUtils.isNotBlank(parts[3])) {
				        		if(set.add(parts[2])) {
					        		writer.write("update we_substitute_customer_order set id_no = '" + sm4.encryptHex(parts[3]) +"' where order_sn = '" + parts[2] + "';");
									writer.newLine();
									writer.flush();
				        		}
				        	}
